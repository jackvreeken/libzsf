...
void __stdcall zsf_calculate(zsf_param_t * p,
                   ...
                   zsf_aux_results_t * aux_results)
{
    ...
    // Flushing discharge
    double flushing_discharge = is_low_tide ? p->flushing_discharge_low_tide : p->flushing_discharge_high_tide;
    ...
    double density_average = 0.5 * (sal_2_density(p->sal_lake, p->temperature, p->rtol, p->atol) + sal_2_density(p->sal_sea, p->temperature, p->rtol, p->atol));
    ...
    while ( 1 )
    {
        ...
        // Leveling
        double mt_lake_1 = volume_leveling_low_tide * p->sal_lake - volume_leveling_high_tide * sal_lock_4;
        ...
        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        double velocity_flushing = flushing_discharge / (p->lock_width * (p->head_lake - p->lock_bottom));

        double sal_diff = sal_lock_2a - p->sal_lake;
        double velocity_exchange_raw = 0.5 * sqrt(g * 0.8 * sal_diff / density_average * (p->head_lake - p->lock_bottom));
        double velocity_exchange_eta = p->density_current_factor_lake * velocity_exchange_raw;
        double frac_lock_exchange = fmax((velocity_exchange_eta - velocity_flushing) / velocity_exchange_eta, 0.0);
        double t_lock_exchange = 2 * p->lock_length / velocity_exchange_eta;
        double volume_exchange_2 = frac_lock_exchange * volume_lock_at_lake * tanh(t_open_lake / t_lock_exchange);
        ...
        double mt_lake_2_flushing = volume_flush_refresh * p->sal_lake + volume_flush_passthrough * p->sal_lake;
        double mt_sea_2_flushing = volume_flush_refresh * sal_lock_2a + volume_flush_passthrough * p->sal_lake;

        // Update state variables of the lock
        double saltmass_lock_2b = saltmass_lock_2a + mt_lake_2_flushing + mt_lake_2_lock_exchange - mt_sea_2_flushing;
        ...
        double mt_lake_2 = mt_lake_2_ship_exit + mt_lake_2_ship_enter + mt_lake_2_flushing + mt_lake_2_lock_exchange;
        ...
        // Leveling
        double mt_sea_3 = volume_leveling_low_tide * sal_lock_2 - volume_leveling_high_tide * p->sal_sea;
        ...
        velocity_exchange_raw = 0.5 * sqrt(g * 0.8 * sal_diff / density_average * (p->head_sea - p->lock_bottom));
        velocity_exchange_eta = p->density_current_factor_sea * velocity_exchange_raw;
        frac_lock_exchange = fmax((velocity_exchange_eta - velocity_flushing) / velocity_exchange_eta, 0.0);
        t_lock_exchange = 2 * p->lock_length / velocity_exchange_eta;
        double volume_exchange_4 = frac_lock_exchange * volume_lock_at_sea * tanh(t_open_sea / t_lock_exchange);
        ...
        double mt_lake_4_flushing = volume_flush_refresh * p->sal_lake + volume_flush_passthrough * p->sal_lake;
        double mt_sea_4_flushing = volume_flush_refresh * sal_lock_4a + volume_flush_passthrough * p->sal_lake;

        // Update state variables of the lock
        double saltmass_lock_4b = saltmass_lock_4a + mt_lake_4_flushing - mt_sea_4_lock_exchange - mt_sea_4_flushing;
        ...
        // ~~~~~~~~~~~~~~~~~~
        // Total mass transports over both gates
        double mt_sea_4 = mt_sea_4_ship_exit + mt_sea_4_ship_enter + mt_sea_4_flushing + mt_sea_4_lock_exchange;
        ...
        if (is_close(sal_lock_4, sal_lock_4_prev, p->rtol, p->atol))
        {
            ...
            double vol_lake_out = (volume_leveling_low_tide + p->ship_volume_sea_to_lake + volume_exchange_2
                            + flushing_discharge * 2 * t_open);
            double disch_lake_out = vol_lake_out / t_cycle;

            double vol_lake_in = (volume_leveling_high_tide + volume_exchange_2 + p->ship_volume_lake_to_sea);
            ...
            double vol_sea_out = (volume_leveling_high_tide + volume_exchange_2 + p->ship_volume_lake_to_sea);
            double disch_sea_out = vol_sea_out / t_cycle;

            double vol_sea_in = (volume_leveling_low_tide + p->ship_volume_sea_to_lake + volume_exchange_2
                          + flushing_discharge * 2 * t_open);
            ...
            if (aux_results != NULL)
            {
                ...
                double velocity_exchange = 0.5 * sqrt(g * 0.8 * sal_diff / density_average * (head_avg - p->lock_bottom));
